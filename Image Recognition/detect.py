...
def send_data(label):
    ...
    try:
        if last_send_time is None or time.time() - last_send_time > 2:
            data = label.encode()
...
def stitch_entry():
    thr = Thread(target=poll_stitch, args=())
    thr.start()
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    print(int(distance) < 25)
                    if (save_img or save_crop or view_img) and conf > conf_thres and int(distance) < 30:  # Add bbox to image
